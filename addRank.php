...
$ID = isset($_POST['ID']) ? $_POST['ID'] : ''; // Initialize userID with empty string if not set
$rows = array(); // Initialize $rows as an empty array

if(isset($_GET['ID'])){
    $ID = $_GET['ID'];
    $sql = "SELECT * FROM individualenrollment WHERE ID=$ID";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $rows = $result->fetch_assoc();
    } else {
        echo "No records found for the given user ID.";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $sql = "UPDATE individualenrollment SET eventName='$eventName', rank='$rank' WHERE ID=$ID";
